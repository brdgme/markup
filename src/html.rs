...
fn fg(color: &Color, content: &str) -> String {
    return format!(r#"<span style="color:{};">{}</span>"#,
                      color,
                      content);
}
...
pub fn render(input: &Vec<Node>, players: &Vec<String>) -> Result<String, MarkupError> {
    transform::transform(input, players)
        ...
        .map(|output| {
            format!(
                r#"<div style="background-color:#ffffff;color:#000000;white-space:pre-wrap;font-family:monospace;">{}</div>"#,
                output,
            )
        })
}
...
fn render_nodes(input: &Vec<Node>) -> Result<String, MarkupError> {
    ...
    for n in input {
        match n {
            ...
            &Node::Fg(ref color, ref children) => buf.push_str(&fg(color, &try!(render_nodes(children)))),
            &Node::Bg(ref color, ref children) => buf.push_str(&bg(color, &try!(render_nodes(children)))),
